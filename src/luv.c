...
static void client_after_close(uv_handle_t *handle)
{
  ...
  // fire 'close' event
  EVENT(self, NULL, EVT_CLOSE, uv_last_error(loop).code, NULL);
  // free self
  client_free(self);
}
...
static void client_after_shutdown(uv_shutdown_t *rq, int status)
{
  ...
  // fire 'shut' event
  EVENT(self, NULL, EVT_SHUT, uv_last_error(loop).code, NULL);
  // close the handle
  client_close(self);
  req_free((uv_req_t *)rq);
}
...
static void client_on_event(client_t *self, msg_t *msg, enum event_t ev, int status, void *data)
{
  ...
  if (ev == EVT_DATA) {
    ...
    DEBUGF("REND %p", msg);
#if DELAY_RESPONSE
    uv_timer_init(loop, &msg->timer_wait);
    msg->timer_wait.data = msg;
    uv_timer_start(&msg->timer_wait, request_on_timeout, DELAY_RESPONSE, 0);
#else
# if SEPARATE_WRITE_HEAD
    response_write_head(msg, RESPONSE_HEAD, after_write);
    response_write(msg, RESPONSE_BODY, after_write);
# else
    response_write_head(msg, RESPONSE_HEAD RESPONSE_BODY, after_write);
# endif
    response_end(msg);
#endif
  } else if (ev == EVT_ERROR) {
...
int main()
{
  ...
  uv_tcp_t server;
  server_init(&server, 8080, "0.0.0.0", 128, client_on_event);
